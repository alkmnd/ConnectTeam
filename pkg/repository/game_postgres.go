...
func (r *GamePostgres) SaveResults(gameId uuid.UUID, userId uuid.UUID, userTempId uuid.UUID, rate int, name string) (id int, err error) {
	if userId != uuid.Nil {
		query := fmt.Sprintf("INSERT INTO %s (game_id, user_id, user_temp_id, value, name) values ($1, $2, $3, $4, $5) RETURNING id", resultsTable)
		row := r.db.QueryRow(query, gameId, userId, userTempId, rate, name)
...
func (r *GamePostgres) GetCreatedGames(page int, userId uuid.UUID) (games []connectteam.Game, err error) {
	query := fmt.Sprintf(`SELECT id, creator_id, invitation_code, name, start_date, status FROM %s g JOIN %s gu ON g.creator_id = gu.user_id WHERE g.creator_id=$1 ORDER BY g.start_date DESC LIMIT $2 OFFSET $3`, gamesTable, gamesUsersTable)
...
func (r *GamePostgres) GetResults(gameId uuid.UUID) (results []models.UserResult, err error) {
	...
	query := fmt.Sprintf(`SELECT id, user_id, user_temp_id, value, name FROM %s WHERE game_id=$1`, resultsTable)
