...
func (r *GamePostgres) GetCreatedGames(page int, userId uuid.UUID) (games []connectteam.Game, err error) {
	query := fmt.Sprintf(`SELECT * FROM %s g JOIN %s gu ON g.creator_id = gu.user_id WHERE g.creator_id=$1 ORDER BY g.start_date DESC LIMIT $2 OFFSET $3`, gamesTable, gamesUsersTable)
