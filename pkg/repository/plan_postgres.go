...
func (r *PlanPostgres) CreatePlan(request connectteam.UserPlan) (connectteam.UserPlan, error) {
	query := fmt.Sprintf(`INSERT INTO %s (user_id, holder_id, expiry_date, duration, plan_access, 
		confirmed, plan_type) VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT (user_id) DO UPDATE SET 
		holder_id = $2, expiry_date = $3, duration = $4, plan_access = $5, confirmed = $6, plan_type = $7 
...
func (r *PlanPostgres) GetUsersPlans() ([] connectteam.UserPlan, error) {
	...
	query := fmt.Sprintf(`SELECT user_id, holder_id, plan_type, 
		plan_access, expiry_date, duration, confirmed FROM %s`, plansUsersTable)
	err := r.db.Select(&plansUsers, query)
	return plansUsers, err
}

func (r *PlanPostgres) SetConfirmed(id int) (error) {
	query := fmt.Sprintf(`UPDATE %s SET confirmed = true, 
		expiry_date = NOW() + interval '1 day' * duration WHERE user_id = %d`, plansUsersTable, id)
